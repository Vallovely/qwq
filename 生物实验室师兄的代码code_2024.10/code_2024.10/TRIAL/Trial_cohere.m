...
figure('name','Coherence between EEG and right TA EMG during left ankle dorsiflexion'),
for i=1:EEGchno-2
    [Cxy,F] = mscohere(EEGepoch(i,:),EMGepoch(2,:),hamming(nfft),noverlap,nfft,samplerate);
...
figure('name','Coherence between EEG and left TA EMG during left ankle dorsiflexion'),
for i=1:EEGchno-2
    [Cxy,F] = mscohere(EEGepoch(i,:),EMGepoch(1,:),hamming(nfft),noverlap,nfft,samplerate);
    subplot(5,7,i),plot(F,Cxy),hold on
    plot([0 samplerate/2],[CriticalThres CriticalThres],'r--')
    title(EEGchlabel{i}),xlim([0 50])
    ylim([0 max(Cxy(5:40))])
