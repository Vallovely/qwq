...
nfft=1024;    % number of frequency bins
...
% EMGdata=filter_2sIIR(EMGdata,EMGpassband,samplerate,forder,'bandpass');
EMGdata=abs(EMGdata);
...
segmentNo=length(segmentSP);    % approximate segment number
...
for i=1:EEGchno-2
    S=[];
    for j=1:segmentNo
        X=[newEEGdata(i,segmentSP(j):segmentSP(j)+nfft-1); EMGdata(2,segmentSP(j):segmentSP(j)+nfft-1)];
